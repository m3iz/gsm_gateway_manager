...
class GsmModem:
    ...
    def connect(self, port: str, baudrate: int = 115200) -> bool:
        ...
        for attempt in range(3):
            ...
            if any('OK' in line for line in resp):
                ...
                # Однократная проверка SIM статуса после подключения
    ...
    def check_sim_status(self) -> str:
        """Check SIM status using AT+CPIN? command."""
        ...
        # Если нет ответа, возможно SIM готова
    ...
    def enter_pin(self, pin: str) -> bool:
        ...
        if any('OK' in line for line in resp):
            # После успешного ввода PIN перепроверяем статус
            self.check_sim_status()
            return True
        else:
            for line in resp:
